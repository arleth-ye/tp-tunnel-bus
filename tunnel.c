...
#define NB_X 5
#define NB_Y 4
// nbr d’allers-retours par bus
#define ALLER_RETOURS 10

// sémaphores pour protéger les variables partagées (compteurs) et gérer les files d’attente
...
void enter_tunnel(char dir) {
    sem_wait(&mutex);        // début sc
    if (dir == 'X') {
        waitingX++;
        // boucle d’attente tq la voie est occupée ou qu’on n’a pas la priorité
    ...
    sem_post(&mutex);        // fin sc
}

/**
 * sortie du tunnel pour un bus de direction `dir`
 * met à jour les compteurs et libère éventuellement la file d’attente opposée en réveillant un thread 
 */
...
int main() {
    ...
    sem_init(&mutex, 0, 1);  // mutex binaire, ouverte initialement
